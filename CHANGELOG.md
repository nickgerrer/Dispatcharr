...
- Personal API keys: Admin users can now generate personal API keys for use in scripts, integrations, and third-party tooling. Keys authenticate via the `Authorization: ApiKey <key>` header or the `X-API-Key: <key>` header.
